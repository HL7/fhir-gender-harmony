...
|Support zero to many instances|Added as optionally repeating|With this cardinality the expectation is there will be a single instance of the extension, but there could be multiple instances of the extension for a single resource if that resource is representing information that has occurred over time and there is a need to capture different values. |Responsibility of template container|
